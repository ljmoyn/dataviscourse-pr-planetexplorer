d3.csv("data/confirmed-planets.csv").then(rawData => {
    ...
    discoveryMethods = discoveryMethods.filter((method, index) => discoveryMethods.indexOf(method) === index);
    discoveryMethods = discoveryMethods.map(function(d) {
        ...
        switch (d) {
            case "Radial Velocity":
                ...
                result.source = "https://exoplanetarchive.ipac.caltech.edu/applications/DocSet/index.html?doctree=/docs/docmenu.xml&startdoc=item_1_01";
                break;
            case "Transit":
            ...
            case "Astrometry":
                ...
                result.source = "https://en.wikipedia.org/wiki/Methods_of_detecting_exoplanets#Astrometry";
                result.notes = ["not direct copy from source.","The space-based observatory Gaia, launched in 2013, is expected to find thousands of planets via astrometry, but prior to the launch of Gaia, no planet detected by astrometry had been confirmed.", "One potential advantage of the astrometric method is that it is most sensitive to planets with large orbits. This makes it complementary to other methods that are most sensitive to planets with small orbits.", "changes in stellar position are so small—and atmospheric and systematic distortions so large—that even the best ground-based telescopes cannot produce precise enough measurements."]
                break;
            case "Disk Kinematics":
                result.description = "Many young stars are surrounded by disks of gas and dust. These disks are typically dominated by keplerian rotation, and embedded planets can disturb the gas flow in identifieable patterns. This was only recently used to discover new exoplanets, first described in a paper published in August 2019."
                result.source = "https://ui.adsabs.harvard.edu/abs/2019NatAs.tmp..419P/abstract"
                break;
            case "Pulsar Timing":
                ...
                result.source = "https://en.wikipedia.org/wiki/Methods_of_detecting_exoplanets#Pulsar_timing"
                break;
            case "Transit Timing Variations":
                ...
                result.source = "https://en.wikipedia.org/wiki/Methods_of_detecting_exoplanets#Transit_timing";
                break;
            case "Pulsation Timing Variations":
                ...
                result.source = "https://en.wikipedia.org/wiki/Methods_of_detecting_exoplanets#Variable_star_timing"
                break;
            case "Orbital Brightness Modulation":
                result.description = "A heavy planet orbiting close to a star can distort the shape of the star in a regular way. When the shape change modifies the area of the star as seen from Earth, the brightness of the star undergoes a regular change, indicating the presence of a planet."
    ...
    facilities = facilities.filter((facility, index) => facilities.indexOf(facility) === index);
});
